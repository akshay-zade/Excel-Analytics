...
const Header = () => {
  ...
  return (
    <header className="bg-white/10  shadow-md">
      <nav className="container mx-auto px-4 py-3 flex justify-between items-center">
        <Link to="/" className="text-2xl font-bold text-blue-600">
         <span className='text-white'>Excel</span> Analytics
        </Link>
        ...
        <div className="flex items-center space-x-6">
          {isAuthenticated ? (
            ...
            user.isAdmin ? (
              // --- ADMIN VIEW ---
              <>
                <span className="text-white-700 font-semibold hidden sm:block">Welcome, {user?.name} (Admin)</span>
                <Link 
                  to="/admin" 
              ...
              <>
                ...
                <Link to="/dashboard" className="text-white hover:text-blue-600 font-medium">Dashboard</Link>
                <button 
                  onClick={onLogout} 
                  className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600 transition-colors font-semibold text-sm"
                >
