    ...
    return (
        <div className="space-y-6 p-4 md:p-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0">
                <h1 className="text-3xl text-white font-bold text-gray-800">
                    Analysis for: <span className="text-blue-600">{currentFile.fileInfo.originalName}</span>
                </h1>
                <Link to="/dashboard" className="bg-white/10 text-white px-4 py-2 rounded-lg font-semibold hover:bg-gray-300 transition-colors">
                    &larr; Back to Dashboard
                </Link>
            </div>

            <div className="bg-white/10 p-4 rounded-lg shadow-md flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
...
const AxisSelector = ({ label, value, onChange, headers }) => (
    <div className="w-36">
        <label className="block text-sm font-medium text-white mr-2">{label}:</label>
        <select value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 block w-full bg-white/10 font-medium text-white pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
            {headers.map(h => <option  key={h} value={h} className="bg-option font-medium">{h}</option>)}
        </select>
    </div>
);
...
const ChartContainer = ({ title, children, chartId }) => {
    ...
    return (
        <>
            <div id={chartId} className={`bg-white/10 p-4 rounded-lg shadow-md flex flex-col ${isExpanded ? 'hidden' : ''}`}>
